...
# This is the path in the filesystem where our code lives. More specifically apriltag/src
src_dir = Path(__file__).resolve().parent
root_proj_dir = src_dir.parent

calibration_file = str(src_dir) + '/camera_params.csv'

# These are the inner corners of the checkerboard width and height.
# Remember, a corner is defined by four intersecting lines, so the outside
# edges are not seen as corners to opencv.
...
for photo in photos:
    ...
    if ret:
        ...
        print("how many corners do we have? {}".format(len(corners)))
        exit(2)
